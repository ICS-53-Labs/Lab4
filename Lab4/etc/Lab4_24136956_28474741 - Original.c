...
//TODO: NAMES
// Fix Read function, perhaps use rioreadnb or recv
#include "csapp.h"
/* WebProxy Lab */
/* Logging:
...
int initClient(char* host, int port, struct sockaddr_in* end_server, int* sock) {
	int clientSock;
	struct hostent* hp;
...
int initServer(int port, int* sock) {
    ...
    Bind(serverSock,(SA*)&server_addr,sizeof(server_addr));
...
if (0) {
	...
	*size = 0;
	...
	content_ptr += strlen("Content-Length: ");
	char* content_end = strpbrk(content_ptr,"\r\n");
	int len = content_end - content_ptr;
	char content_len[MAXLINE];
	strncpy(content_len,content_ptr,len);
	content_len[len] = '\0';
	...
	//while ((res = Rio_readn(*readSock,buffer,MAXLINE)) > 0) {
	while (res > 0) {
		if (*size + res + 1 > alloc) { /* Need more alloc space */
			if(DEBUG)printf("not enough room: needed_size: %d alloc:%d\n",*size + res + 1, alloc);
			alloc = alloc * REALLOC + 1;
			Realloc(buf,alloc);
			if(DEBUG)printf("new alloc: %d\n",alloc);
		}
		...
		*size += res;
		...
		if(content_mode == 1 && content_read >= content_length) {
			return SUCCESS;
		}
		if(0 && res < MAXLINE) {
			return SUCCESS;
...
int getStatus(char* response, int responselen, int* status) {
	char* version[MAXLINE], msg[MAXLINE], code[MAXLINE];
	/* Response is of form Version Code Status */
	sscanf(response, "%s %s %s",version,code,msg);
	*status = atoi(code);
	return SUCCESS;
}
...
int logInfo (struct sockaddr_in* addr, char* uri, int size) {
	...
	if (0) {
		...
		third = (ip >> 8) & 0xFF;
		fourth = ip & 0xFF;
		if(DEBUG)printf("dec:%d.%d.%d.%d",first,second,third,fourth);
		fprintf(log_file,"%s: %d.%d.%d.%d %s %d\n",date,first,second,third,fourth,uri,size);
	}
...
int process(int* connSock, struct sockaddr_in* clientaddr) {
	...
	/* Extract status from first token */
	if(0){
	if(DEBUG) printf("Getting status...\n");
	res = getStatus(response_buffer,responselen,&status);
	if (res != SUCCESS){
		free(request_buffer);
		free(response_buffer);
		return res;
	} 
	if(DEBUG) printf("Finished getting status: %d\n",status);
	if(NFO) printf("*** Status ***\n");
	if(NFO) printf("Status: %d\n",status);
	}
	/* Check status for OK */
	if (status == OK) {
		/* Log information */
